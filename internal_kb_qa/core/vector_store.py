...
from internal_kb_qa.core.models import Chunk
...
def _load_config() -> Any:
    ...
    return Config(os.path.join(project_root, "config.ini"))
...
def _as_dense_list(vector: Any) -> List[float]:
    """将 embedding 向量统一转为 list[float]，兼容 numpy / torch。"""
...
class VectorStore:
    ...
    def insert(self, chunks: List[Chunk]) -> int:
        ...
        dense_vectors = self._embedding_fn(texts)["dense"]
            ...
            rows.append(
                {
                    ...
                    "text": chunk.text,
                    "dense_vector": _as_dense_list(dense_vectors[index]),
...
def insert(chunks: Sequence[Chunk]) -> int:
    """T3 对外接口：Chunk 向量化并写入 Milvus。"""
    return VectorStore().insert(list(chunks))
