...
CODE_EXTENSIONS = {".py", ".java", ".go", ".js", ".ts", ".cpp", ".c", ".cs"}
FENCED_CODE_RE = re.compile(r"```(\w*)\n(.*?)```", re.DOTALL)


def language_from_source(source: str) -> str:
    lower = source.lower()
    for ext, lang in (
        (".py", "python"),
        (".java", "java"),
...
def split_python_by_ast(source: str) -> List[str]:
    """按 Python 顶层类/函数边界切分。"""
    try:
...
class CodeSplitter:
    """按代码结构切分纯代码文件或提取 Markdown 围栏代码块。"""

    def split(self, documents: List[Document]) -> List[Chunk]:
        chunks: List[Chunk] = []
        for document in documents:
            source = document.metadata.get("source", "")
            language = document.metadata.get("language") or language_from_source(str(source))
            base_meta = dict(document.metadata)

            if language == "python" or str(source).lower().endswith(".py"):
                pieces = split_python_by_ast(document.page_content)
            else:
    ...
    def extract_fenced_code_blocks(self, text: str) -> List[Tuple[str, str, str]]:
        """从 Markdown 提取围栏代码块，返回 (language, code, full_block)。"""
